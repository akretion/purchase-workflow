...
class PurchaseOrder(models.Model):
    ...
    @api.multi
    def _compute_partially_delivered(self):
        for rec in self:
            delivered = [x.delivered for x in rec.order_line if x.delivered]
            if delivered:
                rec.partially_delivered = True
            else:
                rec.partially_delivered = False
    ...
    def approve_proposal(self):
        ...
        if data:
            if self._quantity_to_update(data) and self.state in [
                "confirmed",
                "approved",
            ]:
                # We have to reset order because of changed qty
    ...
    def _quantity_to_update(self, data):
