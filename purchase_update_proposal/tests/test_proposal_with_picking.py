...
class Test(common.SavepointCase):
    ...
    # def test_delivered_field(self):
    #     order = self.get_order_with_user(alternate_user=True)
    #     pick = self.order.picking_ids.filtered(lambda s: s.state == "done")
    #     logger.info("%s" % order.order_line[0].delivered)
    #     logger.info("%s" % order.order_line[1].delivered)
    #     product = pick.move_lines[0].product_id
    #     line = order.order_line.filtered(lambda s: s.product_id == product)
    #     assert line.delivered is True
    #     assert order.partially_delivered is True

    def test_date_field(self):
        order = self.get_order_with_user(alternate_user=True)
        assert self.dvd_line.delivered is True
        assert self.other_line.delivered is False
        # DVD product line
    ...
    def test_price_field(self):
        ...
        self.order.approve_proposal()
        assert self.dvd_line.price_unit == 99
        # other product
        self.other_line.button_update_proposal()
    ...
    def test_qty_field(self):
        ...
        assert self.order.proposal_state == "submitted"
        assert self.order.proposal_updatable == "no"

    # def test_populate_all_lines(self):
    #     order = self.get_order_with_user(alternate_user=True)
    #     order.populate_all_purchase_lines()
    #     assert len(order.proposal_ids) == 1
    #     assert order.proposal_ids[0].line_id == self.other_line
